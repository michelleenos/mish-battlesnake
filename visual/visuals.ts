...
// @ts-ignore
const sampleEnemyNextTo2Head: Coord[] = [
    { x: 5, y: 3 },
    { x: 6, y: 3 },
    { x: 7, y: 3 },
    { x: 8, y: 3 },
...
// @ts-ignore
const slightlySmallEnemy: Coord[] = [
    { x: 2, y: 2 },
    { x: 1, y: 2 },
    { x: 1, y: 3 },
    { x: 1, y: 4 },
...
const px = (x: number) => x * cellW
const py = (y: number) => (height - 1 - y) * cellH
...
const foodResults = foodPathResults.filter((r) => r !== null && r.costToNext < 3) as AStarResult[]
